...
class SubBruteGUI:
    ...
    def run_enumeration(self):
        ...
        try:
            ...
            self.root.after(
                0, lambda: self.add_log_entry(
                    "INFO", "Enumeration completed: {} results found".format(
                        len(self.results))))

        except Exception as e:
            self.root.after(
                0, lambda err=e: self.add_log_entry(
                    "ERROR", "Enumeration error: " + str(err)))
            self.root.after(
                0, lambda err=e: messagebox.showerror(
                    "Enumeration Error", "An error occurred: " + str(err)))
