...
class NameServerVerifier(multiprocessing.Process):
    ...
    def __init__(self, target, record_type, resolver_q, resolver_list, wildcards):
        ...
        try:
            ...
            logger.info("NameServerVerifier initialized with {} nameservers to test".format(len(self.resolver_list)))
        ...
        while not self.time_to_die and keep_trying and retry_count < max_retries:
            try:
                ...
                if type(e) == Queue.Full or str(type(e)) == "<class 'queue.Full'>":
                    retry_count += 1
                    logger.debug("Queue full, retrying to add nameserver {nameserver} (attempt {retry_count})")
                    keep_trying = True
                else:
                    logger.error("Unexpected error adding nameserver {nameserver}: {e}")
                    return False
        
        if retry_count >= max_retries:
            logger.warning("Failed to add nameserver {nameserver} after {max_retries} retries")
    ...
    def verify(self, nameserver_list):
        ...
        for server in nameserver_list:
            ...
            if not server:
                logger.debug("Skipping empty nameserver entry ({processed_count}/{total_servers})")
                continue
                
            logger.debug("Testing nameserver {server} ({processed_count}/{total_servers})")
            ...
            try:
                ...
                if test_result:
                    ...
                    if self.find_wildcards(self.target):
                        ...
                        if self.add_nameserver(server):
                            added_resolver = True
                            logger.info("Successfully verified and added nameserver: {server}")
                        else:
                            logger.warning("Failed to add verified nameserver {server} to queue")
                    else:
                        logger.warning("Rejected nameserver {server}: wildcard detection failed")
                else:
                    logger.warning("Rejected nameserver {server}: no response to test query")
                ...
                logger.debug("Rejected nameserver {server}: {type(e).__name__}: {e}")
        
        success_rate = (processed_count / total_servers * 100) if total_servers > 0 else 0
        logger.info("Nameserver verification completed: {success_rate:.1f}% processed, {'success' if added_resolver else 'no valid nameservers found'}")
    ...
    def run(self):
        ...
        try:
            ...
            if self.resolver_list:
                ...
                logger.debug("Shuffled resolver list of {len(self.resolver_list)} nameservers")
    ...
    def find_wildcards(self, host):
        ...
        try:
            ...
            if len(wildtest):
                logger.warning("Spam DNS detected for nameserver {self.resolver.nameservers}: responds to random domains")
                return False
        ...
        while looking_for_wildcards and test_counter >= 0:
            ...
            try:
                ...
                logger.warning("Wildcard detection error for nameserver {self.resolver.nameservers}: {type(e).__name__}: {e}")
                return False
        ...
        if wildcards_found > 0:
            logger.info("Wildcard detection completed: {wildcards_found} wildcard records found for {host}")
        else:
...
class DNSLookupWorker(multiprocessing.Process):
    ...
    def get_nameserver_blocking(self):
        ...
        try:
            ...
            logger.warning("Failed to get nameserver (blocking): {type(e).__name__}: {e}")
            return []
...
class SubBrute:
    ...
    def __init__(self, target, record_type=None, subdomains_file="names.txt", 
                 resolvers_file="resolvers.txt", process_count=16, debug=True):
        """
        ...
        logger.info("SubBrute initialized - Target: {}, Record Type: {}".format(self.target, self.record_type))
        logger.debug("Configuration: {} processes, Debug: {}".format(self.process_count, self.debug))
    ...
    def load_wordlists(self):
        ...
        """
        logger.debug("Loading wordlists - Subdomains: {self.subdomains_file}, Resolvers: {self.resolvers_file}")
        
        try:
            subdomains = check_open(self.subdomains_file)
            resolvers = check_open(self.resolvers_file)
            
            logger.info("Loaded {len(subdomains)} subdomains and {len(resolvers)} resolvers")
            
            # Validate resolver-to-process ratio
            if (len(resolvers) / 16) < self.process_count:
                logger.warning("Only {len(resolvers)/self.process_count:.1f} resolvers per process. Consider adding more nameservers.")
    ...
    def run_enumeration(self):
        ...
        try:
            ...
            resolver_queue = multiprocessing.Queue(maxsize=2)  # Small buffer for nameservers
            ...
            logger.debug("Added target domain {self.target} to processing queue")
            ...
            logger.info("Starting {self.process_count} DNS lookup worker processes")
            workers = []
            for i in range(self.process_count):
                ...
                logger.debug("Started worker process {i+1}/{self.process_count}")
            ...
            while active_workers > 0:
                try:
                    ...
                    if result is False:
                        # Worker finished
                        active_workers -= 1
                        logger.debug("Worker finished, {active_workers} remaining")
                        continue
                    ...
                    if "timeout" in str(e).lower():
                        logger.debug("Result collection timeout, checking worker status")
            ...
            logger.info("Result collection completed: {results_count} successful lookups")
    ...
    def _log_final_stats(self):
        ...
        if self.stats['start_time'] and self.stats['end_time']:
            ...
            logger.info("Enumeration completed in {duration:.2f} seconds")
            logger.info("Statistics: {self.stats['successful_lookups']} successful lookups")
...
def run(target, record_type = None, subdomains = "names.txt", resolve_list = "resolvers.txt", process_count = 16):
    ...
    verify_nameservers_proc = NameServerVerifier(target, record_type, resolve_q, resolve_list, wildcards)
    ...
    for i in range(process_count):
        worker = DNSLookupWorker(in_q, out_q, resolve_q, target, wildcards, spider_blacklist)
        worker.start()
...
class ColoredLogger:
    ...
    def __init__(self, name="SubBrute", log_file="subbrute.log", debug=True):
        """
    ...
    def debug(self, *args):
        ...
        # For Python 2 compatibility, skip complex debug messages
        pass
...
if __name__ == "__main__":
    ...
    parser.add_option("-", "--filter_subs", dest = "filter", default = "",
        ...
        try:
            ...
            for hostname, dns_record_type, addresses in subbrute.run_enumeration():
                ...
                if not dns_record_type:
                    ...
                    formatted_result = "{},{}".format(hostname, ','.join(addresses).strip(','))
            ...
            logger.info("Enumeration completed for {target}: {result_count} results found")
