...
try:
    from subbrute import SubBrute, ColoredLogger
except ImportError:
...
"""
    ...
    def show_result_details(self, hostname):
        ...
        ttk.Label(frame, text="Hostname: {result['hostname']}", font=('Arial', 12, 'bold')).pack(anchor=tk.W, pady=(0, 10))
        ...
        ttk.Button(button_frame, text="Copy All IPs", 
                  command=lambda: self.copy_to_clipboard(', '.join(result['addresses']))).pack(side=tk.LEFT, padx=(0, 5))
...
Target Domain: {self.target_var.get()}
Record Type: {self.record_type_var.get()}
Process Count: {self.process_count_var.get()}

Timing:
- Start Time: {datetime.fromtimestamp(self.stats['start_time']).strftime('%Y-%m-%d %H:%M:%S')}
- End Time: {datetime.fromtimestamp(self.stats['end_time']).strftime('%Y-%m-%d %H:%M:%S') if self.stats['end_time'] else 'Running...'}
- Duration: {int(elapsed//3600):02d}:{int((elapsed%3600)//60):02d}:{int(elapsed%60):02d}
